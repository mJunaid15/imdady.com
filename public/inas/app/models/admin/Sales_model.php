...
class Sales_model extends CI_Model
{
    ...
    public function getProductComboItems($pid, $warehouse_id = NULL)
    {
     ...
     public function getaddsalesbanks()
    {
        $qbanksales = $this->db->get('bank');
        if ($qbanksales->num_rows() > 0) {
            foreach (($qbanksales->result()) as $row) {
                $data[] = $row;
            }
            return $data;
        }
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            if ($data['payment_status'] == 'partial' || $data['payment_status'] == 'paid' && !empty($payment)) {
                ...
                if ($payment['paid_by'] == 'gift_card') {
                    ...
                    $query = "UPDATE bank set current_amount = (current_amount+$amount) WHERE id =  $bank_id";
                    $this->db->query($query);
                } else {
