...
class Purchases_model extends CI_Model
{
    ...
    public function addExpense($data = array())
    {
        ...
        $query = "UPDATE sma_bank SET current_amount = (current_amount	- $amount) WHERE id = $bank_id";
        $query1="Select current_amount from sma_bank where id = $bank_id";
        foreach($this->db->query($query1)->result_array() as $data1):
        endforeach;
        if(($data1['current_amount']-$amount)>=0){
        $this->db->query($query);
        ...
        echo "<script>alert('Your Account have insufficient balance');
        location.href = 'http://satatech.biz/choices/admin/purchases/expenses';</script>";
        }
    }
