...
class Purchases extends MY_Controller
{
    ...
    public function getPurchases($warehouse_id = null)
    {
        ...
        $add_payment_link_by_bank = anchor('http://smartmanagerv2.futuregates.sa/querytest/index.php/welcome/Addbankpayment/$1', '<i class="fa fa-money"></i> ' . lang('add_payment_by_bank'),['target' => '_blank']);
        ...
        <ul class="dropdown-menu pull-right" role="menu">
            ...
            <li>' . $add_payment_link_by_bank . '</li>
            <li>' . $edit_link . '</li>
            <li>' . $pdf_link . '</li>
            <li>' . $email_link . '</li>
            <li>' . $print_barcode . '</li>
...
//generate pdf and force to download
    ...
    public function getExpenses()
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('expenses') . ".id as id, date, reference, {$this->db->dbprefix('expense_categories')}.name as category,{$this->db->dbprefix('bank')}.account_no as bank, amount, note, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as user, attachment", false)
            ->from('expenses')
            ->join('users', 'users.id=expenses.created_by', 'left')
            ->join('expense_categories', 'expense_categories.id=expenses.category_id', 'left')
            ->join('bank', 'bank.id=expenses.bank_id', 'left')
    ...
    public function add_expense()
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addExpense($data)) {
            ...
            $this->data['bank'] = $this->purchases_model->getbanks();
            $this->data['modal_js'] = $this->site->modal_js();
    ...
    public function edit_expense($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updateExpense($id, $data)) {
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['categories'] = $this->purchases_model->getExpenseCategories();
             $this->data['banks'] = $this->purchases_model->getbanks();
