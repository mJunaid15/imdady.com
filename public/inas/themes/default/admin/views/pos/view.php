    ...
    <div id="wrapper">
        <div id="receiptData">
            ...
            <div id="receipt-data">
                ...
                <table class="table table-condensed">
                    <tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr><td colspan="2" class="no-border">#' . $r . ': &nbsp;&nbsp;' . product_name($row->product_name, ($printer ? $printer->char_per_line : null)) . ($row->variant ? ' (' . $row->variant . ')' : '') . '<span class="pull-right">'  . '</span></td></tr>';
                            if (!empty($row->second_name)) {
                                echo '<tr><td colspan="2" class="no-border">' . $row->second_name . '</td></tr>';
                            }       
                            $zaibi = $row->subtotal - $row->item_tax;
                            $zaibi_tax += $row->item_tax;
                            $zaibi_total += $zaibi;
                            if($row->taxes != 0){
                                
                                echo '<tr><td class="no-border border-bottom">' . $this->sma->formatQuantity($row->unit_quantity) . ' x ' . ($row->item_discount != 0 ? '(' . $this->sma->formatMoney($zaibi + $row->item_discount) . ' - ' . $this->sma->formatMoney($row->item_discount) . ')' : $this->sma->formatMoney($zaibi)) . '</td><td class="no-border border-bottom text-right">' . $this->sma->formatMoney($zaibi) . '</td></tr>';


                            }else{
                                
                                //  ($row->item_tax != 0 ? ' [' . lang('tax') . ' <small>(' . ($Settings->indian_gst ? $row->tax : $row->tax_code) . ')</small> ' . $this->sma->formatMoney($row->item_tax) . ($row->hsn_code ? ' (' . lang($row->product_type == 'service' ? 'sac_code' : 'hsn_code') . ': ' . $row->hsn_code . ')' : '') . ']' : '') .
                                
                                echo '<tr><td class="no-border border-bottom">' . $this->sma->formatQuantity($row->unit_quantity) . ' x ' . ($row->item_discount != 0 ? '(' . $this->sma->formatMoney($row->unit_price + $row->item_discount) . ' - ' . $this->sma->formatMoney($row->item_discount) . ')' : $this->sma->formatMoney($row->unit_price)) .'</td><td class="no-border border-bottom text-right">' . $this->sma->formatMoney($zaibi) . '</td></tr>';
                    ...
                    <tfoot>
                        <tr>
                            <th><?=lang('totals');?></th>
                            <th class="text-right"><?=$this->sma->formatMoney($zaibi_total);?></th>
                        </tr>
                        <tr><th><?php echo $row->tax_code;?></th><th class="text-right"><?php  echo $this->sma->formatMoney($zaibi_tax)?> </th></tr>
                        <?php
                        if ($inv->order_tax != 0) {
                            echo '<tr><th>' . lang('vats') . '</th><th class="text-right">' . $this->sma->formatMoney($return_sale ? ($inv->order_tax + $return_sale->order_tax) : $inv->order_tax) . '</th></tr>';
                        }
                ...
                . '<br>' .
                lang('totals') . ' ' . lang('award_points') . ': ' . $customer->award_points . '</p>' : ''; ?>
